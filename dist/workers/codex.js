...
function shouldRetryWithoutSchema(stdout, stderr) {
...
async function executeCodexAttempt(baseCommand, args, timeoutMs, context, runner) {
...
export const codexWorker = {
    ...
    async run(context, runner) {
        ...
        const firstAttempt = await executeCodexAttempt(context.settings.codex_command, firstArgs, timeoutMs, context, runner);
        ...
        if (firstAttempt.status === "error" &&
            ...
            const fallbackAttempt = await executeCodexAttempt(context.settings.codex_command, ["exec", "--json", "--output-last-message", context.task], timeoutMs, context, runner);
