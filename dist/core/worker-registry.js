...
function normalizeTransport(type, value) {
    if (value) {
        return value;
    }
    if (type === "mcp") {
...
function isIncluded(id, sourceName, discovery) {
...
function isExcluded(id, sourceName, discovery) {
...
function normalizeDefinitionEntry(id, definition, source, sourceName) {
...
function createBuiltinWorkers(settings) {
...
function mapLegacyWorkerRegistry(settings) {
    const combined = {};
...
function mapDiscoveredServer(server, settings) {
...
function mapDiscoveredCliCandidates(settings) {
    const discovery = settings.discovery;
    ...
    for (const [id, definition] of Object.entries(discovery?.cli_candidates ?? {})) {
        ...
        const entry = normalizeDefinitionEntry(id, {
            ...definition,
            type: "cli"
        }, "discovered");
        entries.push(entry);
...
function applyDisableList(workers, discovery) {
...
function sortByPriority(workers) {
    return [...workers].sort((left, right) => left.priority - right.priority);
}
export async function buildWorkerRegistry(settings, cwd) {
    ...
    pushEntries(discoveredServers.map((server) => mapDiscoveredServer(server, settings)).filter(Boolean));
    pushEntries(mapDiscoveredCliCandidates(settings));
    const manualWorkers = mapLegacyWorkerRegistry(settings);
    const manualEntries = Object.entries(manualWorkers).map(([id, definition]) => normalizeDefinitionEntry(id, definition, definition.source ?? "manual"));
